...
def Generate_Light(song_name : str): 
    ...
    for MH in Song.measureHeaders:
        ...
        def Note_To_Color(pitch, velocity):
            hue = (pitch % 12) / 12 + random.uniform(-0.5, 0.5)
            saturation = random.uniform(0.3, 1)
            value = 0.4 + (velocity / 127) * 0.6 + random.uniform(-0.4, 0.4)
            r, g, b = colorsys.hsv_to_rgb(hue % 1, saturation, value)
            return int(r * 255), int(g * 255), int(b * 255)

        for measure in [Measure_Guitar, Measure_Bass, Measure_Bass_Drive, Measure_Drums]:
            for beat in measure.voices[0].beats:
                for note in beat.notes:
                    pitch = note.value
                    velocity = note.velocity
                    red, green, blue = Note_To_Color(pitch, velocity)
                    tick_start = beat.start
                    tick_duration = QUARTER_NOTE_TICKS // beat.duration.value
                    if random.random() > 0.3:
                        tick_duration *= random.choice([0.1, 0.3, 0.5, 1.5, 2, 3])
                        red = int(red * random.uniform(0.2, 2)) % 256
                        green = int(green * random.uniform(0.2, 2)) % 256
                        blue = int(blue * random.uniform(0.2, 2)) % 256
                    Light_Changes.append(Light_Change(measure.track.name, tick_start, tick_duration, red, green, blue))
                    if random.random() > 0.8:
                        Light_Changes.append(Light_Change(measure.track.name, tick_start + tick_duration // 2, tick_duration, blue, red, green))
