...
namespace MIDILightDrawer
{
	...
	void Form_MIDI_Log::Initialize_Components()
	{
		...
		_Grid_Log->ForeColor = Theme_Manager::Get_Instance()->ForegroundText;
	...
	void Form_MIDI_Log::Add_Log_Entry(MIDI_Log_Entry^ entry)
	{
		...
		// Add to grid (invoke if needed for thread safety)
		if (this->InvokeRequired)
		{
			this->Invoke(gcnew Action<MIDI_Log_Entry^>(this, &Form_MIDI_Log::Add_Log_Entry), entry);
			return;
		}
	...
	void Form_MIDI_Log::Add_MIDI_Event(double timestamp_ms, int track, int channel,
